...
def student_profile():
    ...
    try:
        ...
        logging.error(f"Ошибка профиля ученика: {str(e)}")
        return render_template('error.html', error="Ошибка загрузки профиля"), 500
...
def handle_connect():
    if 'user' in session:
        emit('user_connected', {
            ...
            'is_admin': session.get('is_admin', False)  # Исправлено
        })
